...
options.samplename = 'test-karyolib-4';

options.seqtype = 'cg';
options.infile = '/data/suzaku/yau/cg/data/aaron/summary/snps-100.txt';
options.outdir = '.';
...
%	case 'illumina'
%		[chr, arm, pos, k, d, dd] = loaddata(options);
%	case 'cg'
%		[chr, arm, pos, k, d, dd] = loaddataCG(options);
%end

load('/data/suzaku/yau/cancercopy/data/testex_1_3.mat');
length(chr)
...
%%	save testdat -append x u seg log_pr_s;
