...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    int i, j, k, m;
    int nStates, nSnps;
    double *logobsMatrix, *logNu, *logtransMat; 
	mxArray *tmpVec;
    ...
    /* Get the data. */
	logNu 				= (double *)mxGetPr(prhs[0]);
	logtransMat 		= (double *)mxGetPr(prhs[1]);
    logobsMatrix        = (double *)mxGetPr(prhs[2]);
    ...
    for ( k = 1; k < nSnps; k++ ) {
		...
		for ( j = 0; j < nStates; j++ ) {
			...
			for ( i = 0; i < nStates; i++ ) {
				tmp[i] = phi[nStates*(k-1) + i] + logobsMatrix[nStates*k + j] + logtransMat[j*nStates + i];
			}
